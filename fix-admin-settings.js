...
// 1. Add Tab Button
const searchTabs = '<button class="tab-btn" data-tab="tab-reviews">⭐ Reseñas</button>';
const replaceTabs = searchTabs + '\n            <button class="tab-btn" data-tab="tab-settings">⚙️ Configuración</button>';
if (!html.includes('tab-settings')) {
    html = html.replace(searchTabs, replaceTabs);
}
...
if (!html.includes('id="tab-settings"')) {
    ...
    if(closingDiv !== -1) {
        ...
        console.log('Settings tab added to admin.html');
    }
} else {
